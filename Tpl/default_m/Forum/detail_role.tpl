<php>$item_list = ff_mysql_forum('pid:'.$forum_id.';limit:20;page_is:true;page_id:forum;page_p:'.$forum_page.';cache_name:default;cache_time:default;order:forum_addtime;sort:desc');
$page_total = ff_page_count('forum', 'totalpages');
if($forum_cid){
	$info = reset(ff_mysql_role('ids:'.$forum_cid.';limit:1;cache_name:default;cache_time:default;order:person_id;sort:desc'));
}</php><!DOCTYPE html>
<html lang="zh-cn">
<head>
<include file="BlockTheme:header_meta" />
<include file="./Tpl/base/seo/forum_detail_role" />
</head>
...
<div class="page-header">
  <h2 class="text-ellipsis">
    ...
    <a href="{:ff_url('role/forum', array('id'=>$forum_cid), true)}">《{$info.person_name|msubstr=0,36,true}》的话题</a>
  </h2>
</div>
<p class="content">
  {$forum_content|htmlspecialchars|nb}
	<a class="text-green" href="{:ff_url('role/forum', array('id'=>$forum_cid), true)}">@{$info.person_name}</a>
</p>
...
<p class="text-center">
  ...
  <a class="btn btn-default ff-updown-set" href="javascript:;" data-id="{$forum_id}" data-module="forum" data-type="down" data-toggle="tooltip" data-placement="top" title="反对">
	...
	<a class="btn btn-default ff-updown-set" href="{:ff_url('role/read',array('id'=>$forum_cid),true)}">
	<span class="glyphicon glyphicon-link text-green"></span> 阅读
	</a>
</p>
<!-- -->
<div class="page-header">
  <h2>
		<span class="glyphicon glyphicon-comment text-green"></span> 发表您的看法
	</h2>
</div>
<!--发表评论后刷新网页 -->
