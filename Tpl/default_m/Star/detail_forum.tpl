...
$page_info = ff_url_page('star/forum', $jump, true, $params['page_id'], 4);
$page_total = ff_page_count($params['page_id'], 'totalpages');
</php><!DOCTYPE html>
<html lang="zh-cn">
<head>
<include file="BlockTheme:header_meta" />
<include file="./Tpl/base/seo/star_forum" />
</head>
...
<div class="ff-forum-reload">
	<form class="form-forum ff-forum-post" role="form" action="{$root}index.php?s=forum-update" method="post">
		...
		<input name="forum_sid" type="hidden" value="8" />
		<input name="forum_pid" type="hidden" value="0" />
		<div class="form-group">
			<textarea name="forum_content" class="form-control" rows="5" placeholder="吐槽{$person_name}......"></textarea>
		</div>
...
<div class="text-center">
  <ul class="pager">
    <gt name="params.page_p" value="1">
      <li><a id="ff-prev" href="{:ff_url('star/forum', array('id'=>$person_id,'p'=>($params['page_p']-1)), true)}">上一页</a></li>
    </gt>
    <lt name="params.page_p" value="$page_total">
      <li><a id="ff-next" href="{:ff_url('star/forum', array('id'=>$person_id,'p'=>($params['page_p']+1)), true)}">下一页</a></li>
    </lt>
  </ul>
 </div>
</gt>
