...
$jump = array('id'=>$news_id,'p'=>'FFLINK');
$item_list = ff_mysql_forum($params);
$page_info = ff_url_page('news/forum', $jump, true, $params['page_id'], 4);
$page_total = ff_page_count($params['page_id'], 'totalpages');
</php><!DOCTYPE html>
<html lang="zh-cn">
<head>
<include file="./Tpl/base/bootstrap3/inc_header" />
<include file="./Tpl/base/seo/news_forum" />
</head>
...
<div class="page-header">
  ...
  <a href="{:ff_url_read_news($list_id,$list_dir,$news_id,$news_ename,$news_jumpurl)}">{$news_name|msubstr=0,20,true}</a>
...
<div class="ff-forum-reload">
	<form class="form-forum ff-forum-post" role="form" action="{$root}index.php?s=forum-update" method="post">
		<input name="forum_cid" type="hidden" value="{$news_id}" />
...
<gt name="page_total" value="1">
  ...
  <ul class="pager visible-xs visible-sm">
    <gt name="params.page_p" value="1">
      <li><a id="ff-prev" href="{:ff_url('news/forum', array('id'=>$news_id,'p'=>($params['page_p']-1)), true)}">上一页</a></li>
    </gt>
    <lt name="params.page_p" value="$page_total">
      <li><a id="ff-next" href="{:ff_url('news/forum', array('id'=>$news_id,'p'=>($params['page_p']+1)), true)}">下一页</a></li>
    </lt>
  </ul>
  </div>
</gt>
