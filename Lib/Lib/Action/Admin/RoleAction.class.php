...
class RoleAction extends BaseAction{	
  ...
  public function show(){
		...
		$admin['father_id']= $_REQUEST['father_id'];
		$admin['object_id']= $_REQUEST['object_id'];
		...
		// 数据查询
		$list = ff_mysql_role(array_merge($admin,array('order'=>'person_'.$admin['order'])));
		foreach($list as $key=>$val){
		  $list[$key]['list_url'] = '?s=Admin-Role-Show-cid-'.$list[$key]['person_cid'];
		...
		$page['pages'] = '共'.$page['records'].'个角色&nbsp;当前:'.$page['currentpage'].'/'.$page['totalpages'].'页&nbsp;'.getpage($page['currentpage'],$page['totalpages'],8,$page['jump'],'pagego(\''.$page['jump'].'\','.$page['totalpages'].')');
		//变量附值
		$this->assign($urls);
		$this->assign($page);
		...
		//加载模板
		$this->display('./Public/system/role_show.html');
  }
	// 添加编辑
  public function add(){
		$id = intval($_GET['id']);
  ...
  public function pestcid(){
	...
	public function ename(){
		...
		F('_feifeicms/enamerole',$person_id);
		$this->redirect('Role/Ename', array('minid'=>$person_id,'maxlen'=>$maxlen), C('collect_time'),'<p>页面跳转中~</p>');
	}			
}
?>